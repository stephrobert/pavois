...
// hasControls reports whether a profile directory holds at least one control.
//
// A profile directory can exist and be empty. In this repository that is the NORMAL state of a
// fresh checkout: profiles/linux/<os>/inspec.yml and waivers.yml are tracked, and controls/*.rb are
// generated by `mise run render` and gitignored. So `profiles/linux/debian12` exists, looks like a
// profile, and contains nothing to evaluate.
func hasControls(dir string) bool {
...
func ResolveProfile(root, profile string) (string, error) {
	...
	if fi, err := os.Stat(bundled); err == nil && fi.IsDir() {
		// An EMPTY profile directory must never win over a corpus this binary is carrying.
		//
		// A released binary run from inside a checkout used to resolve here, find the tracked
		// inspec.yml with no controls beside it, and evaluate nothing. The scan then refused to
		// grade, correctly, with "the scan evaluated 0 controls" and a suggestion to render the
		// corpus, while the rules it needed were compiled into the binary printing the message.
		// For a compliance tool that is the worst kind of defect: the answer was in hand and the
		// tool said it had none.
		if hasControls(bundled) || !corpus.Has(profile) {
			return bundled, nil
		}
		// Not silent. Which corpus produced a verdict is part of the verdict, and a developer who
		// edited rules.yml and forgot to render must not be told their change took effect.
		_, _ = fmt.Fprintf(os.Stderr,
			"pavois: %s has no controls, using the corpus embedded in this binary.\n"+
				"        Run `mise run render` to evaluate the working tree's rules instead.\n",
			bundled)
