...
control 'kconfig-gcc-plugin-randstruct' do
  ...
  describe command("grep -h '^CONFIG_GCC_PLUGIN_RANDSTRUCT=' /boot/config-$(uname -r) 2>/dev/null; zcat /proc/config.gz 2>/dev/null | grep '^CONFIG_GCC_PLUGIN_RANDSTRUCT='") do
    its('stdout') { should match(/^CONFIG_GCC_PLUGIN_RANDSTRUCT=y$/) }
  end
end
...
control 'kconfig-gcc-plugin-structleak-byref-all' do
  ...
  describe command("grep -h '^CONFIG_GCC_PLUGIN_STRUCTLEAK_BYREF_ALL=' /boot/config-$(uname -r) 2>/dev/null; zcat /proc/config.gz 2>/dev/null | grep '^CONFIG_GCC_PLUGIN_STRUCTLEAK_BYREF_ALL='") do
    its('stdout') { should match(/^CONFIG_GCC_PLUGIN_STRUCTLEAK_BYREF_ALL=y$/) }
  end
end
