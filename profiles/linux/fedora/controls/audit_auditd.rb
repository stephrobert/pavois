...
control 'audit-etc' do
  impact 0.5
  title 'Record Events that Modify User/Group Information via open syscall - /etc/group'
  tag domain: 'Audit (auditd)'
  tag evidence: 'effective-runtime'
  tag reboot: 'yes'
  tag ssg: 'audit_rules_etc_group_open'
  describe command('auditctl -l') do
    its('stdout') { should match(/(-k +|key=)user\-modify\b/) }
  end
  describe command("grep -rhwsF 'user-modify' /etc/audit/rules.d/*.rules /etc/audit/audit.rules 2>/dev/null") do
    its('stdout') { should match(/\S/) }
  end
end
...
control 'audit-execution-chacl' do
  ...
  tag reboot: 'yes'
  tag bp28: ['R73', 'R33']
  tag cis: ['6.3.3.26', '6.3.3.24', '6.3.3.25', '6.3.3.10', '6.3.3.28', '6.3.3.27']
  tag nist: '3.1.7'
...
control 'audit-kernel-module-loading' do
  ...
  tag reboot: 'yes'
  tag bp28: 'R73'
  tag cis: ['6.3.3.31', '6.3.3.30', '6.3.3.29']
  tag nist: '3.1.7'
  tag level_bp28: 'enhanced'
  tag level_cis: '2'
...
control 'audit-login-events' do
  ...
  tag level_bp28: 'enhanced'
  tag level_cis: '2'
  tag ssg: 'audit_rules_login_events'
  describe command('auditctl -l') do
...
control 'audit-networkconfig-modification' do
  ...
  tag evidence: 'effective-runtime'
  tag reboot: 'yes'
  tag cis: ['6.3.3.7', '6.3.3.6', '6.3.3.8', '6.3.3.9', '6.3.3.5']
  tag nist: ['3.1.7', 'AC-6(9)', 'AU-12(c)', 'AU-2(d)', 'CM-6(a)']
  tag level_cis: '2'
...
control 'audit-unsuccessful-file-modification' do
  ...
  tag level_bp28: 'enhanced'
  tag level_cis: '2'
  tag ssg: 'audit_rules_unsuccessful_file_modification_chmod'
  describe command('auditctl -l') do
...
control 'audit-usergroup-modification' do
  ...
  tag reboot: 'yes'
  tag bp28: 'R73'
  tag cis: ['6.3.3.12', '6.3.3.14', '6.3.3.16', '6.3.3.15', '6.3.3.17', '6.3.3.13']
  tag('pci-dss' => '10.2.1.5')
