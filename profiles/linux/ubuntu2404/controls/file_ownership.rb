...
control 'filegroupowner-journalctl' do
  impact 0.5
  title 'Verify Groupowner on the journalctl command'
  tag domain: 'File ownership'
  tag evidence: 'filesystem-state'
  tag reboot: 'yes'
  tag stig: 'UBTU-24-700050'
  tag ssg: 'file_groupowner_journalctl'
...
control 'filegroupowner-sshd-drop-in-config' do
  impact 0.5
  ...
  tag nist: ['AC-17(a)', 'AC-6(1)', 'CM-6(a)']
  tag level_cis: '1'
  tag ssg: 'file_groupowner_sshd_drop_in_config'
...
control 'filegroupownerships-var-log-landscape' do
  impact 0.5
  ...
  tag cis: '6.1.4.1'
  tag level_cis: '1'
  tag ssg: 'file_groupownerships_var_log_landscape'
...
control 'fileowner-journalctl' do
  impact 0.5
  title 'Verify Owner on the journalctl Command'
  tag domain: 'File ownership'
  tag evidence: 'filesystem-state'
  tag reboot: 'yes'
  tag stig: 'UBTU-24-700040'
  tag ssg: 'file_owner_journalctl'
  only_if { file('/usr/bin/journalctl').exist? }
  describe file('/usr/bin/journalctl') do
    its('uid') { should eq 0 }
  end
...
control 'fileowner-sshd-drop-in-config' do
  impact 0.5
  ...
  tag nist: ['AC-17(a)', 'AC-6(1)', 'CM-6(a)']
  tag level_cis: '1'
  tag ssg: 'file_owner_sshd_drop_in_config'
  only_if { file('/etc/ssh/sshd_config.d').exist? }
  describe file('/etc/ssh/sshd_config.d') do
    its('uid') { should eq 0 }
  end
...
control 'fileownerships-var-log-landscape' do
  impact 0.5
  ...
  tag cis: '6.1.4.1'
  tag level_cis: '1'
  tag ssg: 'file_ownerships_var_log_landscape'
  only_if { file('/var/log/landscape').exist? }
  describe file('/var/log/landscape') do
...
control 'ownership-library-dirs' do
  ...
  tag evidence: 'filesystem-state'
  tag reboot: 'yes'
  tag nist: ['CM-5(6)', 'CM-5(6).1', 'CM-6(a)', 'AC-6(1)']
  tag stig: 'UBTU-24-300007'
