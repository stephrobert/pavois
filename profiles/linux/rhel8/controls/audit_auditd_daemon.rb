...
control 'auditd-disk-full-action' do
  ...
  describe command('grep -qiE \'^[[:space:]]*disk_full_action[[:space:]]*=[[:space:]]*(single|halt)\b\' /etc/audit/auditd.conf 2>/dev/null && echo ok || echo ko') do
    its('stdout.strip') { should eq 'ok' }
  end
end
...
control 'auditd-max-log-file' do
  ...
  describe command('v=$(grep -iE \'^[[:space:]]*max_log_file[[:space:]]*=\' /etc/audit/auditd.conf 2>/dev/null | grep -oE \'[0-9]+\' | tail -1); { [ -n "$v" ] && [ "$v" -ge 8 ] && echo ok; } || echo ko') do
    its('stdout.strip') { should eq 'ok' }
  end
end
