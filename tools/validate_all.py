...
for os in OSES:
    r = subprocess.run(
        [sys.executable, str(ROOT / "tools/cross_validate.py"), os, "--json"],
        capture_output=True,
        text=True,
    )
    try:
        ...
        d = {
            ...
            "pavois_covered": 0,
            "coverage_pct": None,
            "sources": {"ssg": 0, "ansible_lockdown": 0},
            "gaps": [],
            "suspect_mappings": [],
        }
    ...
    rows.append(d)
print(
    f"{'OS':<12} {'SSG':>4} {'AL':>4} {'consensus':>9} {'pavois':>7} "
    f"{'cov%':>6} {'gaps':>5} {'suspect':>7}  note"
)
for d in rows:
    s = d.get("sources", {})
    note = (
        "no CIS benchmark"
        if d["os"] in NO_CIS_BENCHMARK
        else ("SSG immature" if 0 < s.get("ssg", 0) < 50 else "")
    )
    print(
        f"{d['os']:<12} {s.get('ssg', 0):>4} {s.get('ansible_lockdown', 0):>4} "
        f"{d.get('consensus', 0):>9} {d.get('pavois_covered', 0):>7} "
        f"{str(d.get('coverage_pct', '-')):>6} {len(d.get('gaps', [])):>5} "
        f"{len(d.get('suspect_mappings', [])):>7}  {note}"
    )
